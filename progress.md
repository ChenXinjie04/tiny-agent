...
📍 **M1 最小对话回路** —— 已跑通"输入 → 模型 → 输出"的最小闭环，并已把 DeepSeek 客户端抽到 `src/llm/client.ts`。
下一步：进入 **M2 工具调用循环**（定义工具 schema → agent loop）。
...
### M2 — Agent 工具调用循环
- [ ] 定义工具的描述（function-calling schema）
- [ ] 实现 agent loop：模型请求调工具 → 执行 → 结果回传 → 继续
- [ ] 处理多步工具调用直到模型给出最终答复
